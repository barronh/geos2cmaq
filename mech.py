...
def mechext(inpath):
    ...
    spc_data = {}
    for k in list(spc_files.keys()):
        path = spc_files[k]
        spc_dat = dict([(key, eval(v)) for key, v in [l.strip().replace('/','').replace(' ', '').replace(',', '').split("'")[1:3] for l in open(path, 'rt').read().split('\n') if "'" in l]])
        spc_data[k] =  spc_dat
    return spc_data
...
def mechnml(inpath):
    ...
    for k in list(spc_files.keys()):
        ...
        if datstr == '':
            ...
            name_wt = [line.split(':')[:2] for line in datstr.split('\n') if line.strip() != '']
            name_wt = [(name, eval(wt)) for name, wt in name_wt]
            spc_dat = dict(name_wt)
            spc_data[k] = spc_dat 
    return spc_data
...
def mechinc(indict, convpath):
    ...
    for spc_typ, spc_dat in indict.items():
        if spc_typ == 'AE':
            unit = "micrograms/m**3".ljust(16)
        else:
            unit = 'ppmV'.ljust(16)
        for spcn, spcw in spc_dat.items():
