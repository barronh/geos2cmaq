...
def trymap(spc_data, mappath, mapo):
    outf = open(mappath, 'w')
    for spc_typ, spc_dat in list(spc_data.items()):
        for spcn, spcw in list(spc_dat.items()):
...
def map(spc_data, mappath, mapo, profo):
    ...
    # old mapping reader - [kv for kv in [kv.split(',') for kv in open(mappath).read().strip().split('\n')[1:] if kv[0] != '#'] if len(kv) > 1 and kv[-1].strip() != '']
    ks = set([k for k, v in intext])
    mechspcs = set(reduce(list.__add__, [list(spc_dat.keys()) for spc_dat in list(spc_data.values())]))
    mapped = list(ks)
    mapped.sort()
...
if __name__ == '__main__':
    ...
    list(map(mech('testdata'), 'mapping/saprc07tb.csv', go, po))
